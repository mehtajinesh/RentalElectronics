                ...
                !loading && <Slider {...settings}>
                    {
                        items && items.map((itemData) => (
                                <div className="p-2" key={itemData.productID._id}>

                                        <div className="card">
                                            <Link to={`/viewItem/${itemData.productID._id}`} className="text-decoration-none">
                                            <img src={itemData.productID.productImages[0]} className="card-img-top p-4"/>
                                            <div className="card-body">
                                                <div className="fs-5 card-title text-black">{itemData.productID.productName}</div>
                                            </div>
                                            </Link>
                                            <div className="card-footer">
                                                <div className="d-flex flex-column justify-content-between">
                                                    <span className="text-muted my-auto fs-6">Seller:
                                                        <Link to={`/profile/${itemData.userID._id}`} className="text-decoration-none">
